    ...
    <!-- [ 加载状态  ] End -->
        ...
        <div class="col-xl-12" style="padding-top: 15px;">
            <div class="card">
					...
					<form action="synergymx/list" method="post" name="Form" id="Form">
										...
										<c:when test="${not empty varList}">
											<c:forEach items="${varList}" var="var" varStatus="vs">
												<tr>
													<td>
													  	<div class="checkbox d-inline">
                                                        	<input type="checkbox" id="zcheckbox${vs.index }" name='ids' value="${var.SYNERGYMX_ID}">
													...
													<td>
													<c:if test="${'no' == isEnd }">
														<a title="修改" onclick="edit('${var.SYNERGYMX_ID}');" style="cursor:pointer;"><i class="feather icon-edit-2"></i></a>
					                 				</c:if>		
					                 					<a title="删除" onclick="del('${var.SYNERGYMX_ID}');" style="cursor:pointer;"><i class="feather icon-x"></i></a>
													</td>
												</tr>
...
<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>synergymx/goAdd?SYNERGY_ID=${pd.SYNERGY_ID}';
			 diag.Width = 800;
			 diag.Height = 600;
		...
		function del(Id){
			swal({
            	...
            	if (willDelete) {
	            	$.ajax({
	        			type: "POST",
	        			url: '<%=basePath%>synergymx/delete',
	        	    	data: {SYNERGYMX_ID:Id,tm:new Date().getTime()},
	        			dataType:'json',
	        			cache: false,
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>synergymx/goEdit?SYNERGYMX_ID='+Id;
			 diag.Width = 800;
			 diag.Height = 600;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
            	...
            	if (willDelete) {
	    			...
	    			if(str==''){
	    				...
	    				if(msg == '确定要删除选中的数据吗?'){
	    					$.ajax({
	    						type: "POST",
	    						url: '<%=basePath%>synergymx/deleteAll?tm='+new Date().getTime(),
	    				    	data: {DATA_IDS:str},
	    						dataType:'json',
	    						cache: false,
	    						success: function(data){
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>synergymx/excel';
		}

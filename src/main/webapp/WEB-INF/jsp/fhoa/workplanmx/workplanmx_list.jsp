										...
										<c:when test="${not empty varList}">
											<c:forEach items="${varList}" var="var" varStatus="vs">
												<tr>
													...
													<td>
													<c:if test="${'no' == isEnd }">	
														<a title="修改" onclick="edit('${var.WORKPLANMX_ID}');" style="cursor:pointer;"><i class="feather icon-edit-2"></i></a>
					                 				</c:if>	
					                 				    <a title="删除" onclick="del('${var.WORKPLANMX_ID}');" style="cursor:pointer;"><i class="feather icon-x"></i></a>
													</td>
												</tr>
...
<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>workplanmx/goAdd?WORKPLAN_ID=${pd.WORKPLAN_ID}';
			 diag.Width = 800;
			 diag.Height = 600;

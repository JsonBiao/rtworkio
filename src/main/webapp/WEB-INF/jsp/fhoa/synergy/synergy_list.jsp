                            ...
                            <div class="row">
                                ...
                                <div class="col-xl-12">
                                    <div class="card">
										...
										<form action="synergy/list" method="post" name="Form" id="Form">
											<!-- 检索  -->
														...
														<c:choose>
															<c:when test="${not empty varList}">
																<c:forEach items="${varList}" var="var" varStatus="vs">
																	<tr ondblclick="edit('${var.SYNERGY_ID}');">
																		...
																		<td>${var.OPINIONS}</td>
																		<td>
																			<shiro:hasPermission name="synergy:edit"><a title="修改" onclick="edit('${var.SYNERGY_ID}');" style="cursor:pointer;"><i class="feather icon-edit-2"></i></a></shiro:hasPermission>
										                 					<shiro:hasPermission name="synergy:del"><a title="删除" onclick="del('${var.SYNERGY_ID}');" style="cursor:pointer;"><i class="feather icon-x"></i></a></shiro:hasPermission>
																		</td>
																	</tr>
																</c:forEach>
															</c:when>
													...
													<table style="width:100%;margin-top:15px;">
														<tr>
															<td style="vertical-align:top;">
																<shiro:hasPermission name="synergy:add"><a class="btn btn-light btn-sm" onclick="add();">新增</a></shiro:hasPermission>
															</td>
...
<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>synergy/goAdd';
			 diag.Width = 1000;
			 diag.Height = 800;
		...
		function del(Id){
			swal({
            	...
            	if (willDelete) {
	            	$.ajax({
	        			type: "POST",
	        			url: '<%=basePath%>synergy/delete',
	        	    	data: {SYNERGY_ID:Id,tm:new Date().getTime()},
	        			dataType:'json',
	        			cache: false,
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>synergy/goEdit?SYNERGY_ID='+Id;
			 diag.Width = 1000;
			 diag.Height = 800;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>synergy/excel';
		}

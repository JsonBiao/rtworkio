                        ...
                        <div class="page-wrapper">
                            ...
                            <div class="row">
                                ...
                                <div class="col-xl-12">
									...
									<form action="conclusion/list" method="post" name="Form" id="Form">
									<!-- 检索  -->
												...
												<c:choose>
													...
													<!--			<td>${var.CONTENT}</td>-->
																...
																<c:if test="${'no' == var.isEnd }">	
																	<shiro:hasPermission name="conclusion:edit"><a title="修改" onclick="edit('${var.CONCLUSION_ID}');" style="cursor:pointer;"><i class="feather icon-edit-2"></i></a></shiro:hasPermission>
								                 				</c:if>	
								                 					<shiro:hasPermission name="conclusion:del"><a title="删除" onclick="del('${var.CONCLUSION_ID}');" style="cursor:pointer;"><i class="feather icon-x"></i></a></shiro:hasPermission>
																</td>
															</tr>
...
<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>conclusion/goAdd';
			 diag.Width = 1600;
			 diag.Height = 1550;
		...
		function contentmx(id){
			 ...
			 diag. ShowMaxButton = true;	//最大化按钮
		     diag.ShowMinButton = true;		//最小化按钮
			 diag.URL = '<%=basePath%>conclusion/contentDetails?PROC_INST_ID_='+id+'&isRun=0';
			 diag.Width = 1600;
			 diag.Height = 1550;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
		...
		function del(Id){
			swal({
            	...
            	if (willDelete) {
	            	$.ajax({
	        			type: "POST",
	        			url: '<%=basePath%>conclusion/delete',
	        	    	data: {CONCLUSION_ID:Id,tm:new Date().getTime()},
	        			dataType:'json',
	        			cache: false,
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>conclusion/goEdit?CONCLUSION_ID='+Id;
			 diag.Width = 1600;
			 diag.Height = 1550;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
            	...
            	if (willDelete) {
	    			...
	    			if(str==''){
	    				...
	    				if(msg == '确定要删除选中的数据吗?'){
	    					$.ajax({
	    						type: "POST",
	    						url: '<%=basePath%>conclusion/deleteAll?tm='+new Date().getTime(),
	    				    	data: {DATA_IDS:str},
	    						dataType:'json',
	    						cache: false,
	    						success: function(data){
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>conclusion/excel';
		}
